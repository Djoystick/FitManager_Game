-- 00038_fix_market_and_numeric_overflow.sql
BEGIN;

-- 1. Fix numeric field overflow by increasing precision for TON balances
-- TON total supply is ~5.1 Billion. 5,100,000,000 has 10 digits.
-- NUMERIC(18,9) only allows 9 digits before the decimal point (up to 999,999,999).
-- We increase to NUMERIC(20,9) to support up to 99,999,999,999 TON.

ALTER TABLE public.users 
  ALTER COLUMN balance_ton TYPE NUMERIC(20,9);

ALTER TABLE public.treasury 
  ALTER COLUMN prize_pool_ton TYPE NUMERIC(20,9),
  ALTER COLUMN maintenance_ton TYPE NUMERIC(20,9);

ALTER TABLE public.market_listings
  ALTER COLUMN price_ton TYPE NUMERIC(20,9);

-- Drop functions first so we can recreate them with the new signature if necessary (though they use NUMERIC(18,9) internally)
DROP FUNCTION IF EXISTS public.list_player_on_market(UUID, UUID, NUMERIC);
DROP FUNCTION IF EXISTS public.buy_player_from_market(UUID, UUID);


-- 2. Fix list_player_on_market INSERT syntax error
CREATE OR REPLACE FUNCTION public.list_player_on_market(
    p_seller_id UUID,
    p_player_id UUID,
    p_price_ton NUMERIC(20,9)
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    -- Create Listing (FIXED INSERT SYNTAX)
...
-- 3. Recreate buy_player_from_market with updated NUMERIC(20,9) vars
...
DECLARE
    ...
    v_seller_share NUMERIC(20,9);
    v_prize_pool_share NUMERIC(20,9);
    v_maintenance_share NUMERIC(20,9);
BEGIN
    -- Lock listing
    ...
    -- Calculate Splits
    v_seller_share := ROUND((v_listing_row.price_ton * 0.95)::numeric, 9)::numeric(20,9);
    v_prize_pool_share := ROUND((v_listing_row.price_ton * 0.03)::numeric, 9)::numeric(20,9);
    v_maintenance_share := v_listing_row.price_ton - v_seller_share - v_prize_pool_share;
