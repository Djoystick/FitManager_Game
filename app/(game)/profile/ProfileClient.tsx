  ...
  const handleSaveAvatar = () => {
    ...
    startTransition(async () => {
      ...
      if (res.success) {
        ...
        toast.error(res.error === 'error_insufficient_fc' ? (t.error_insufficient_fc || 'Not enough FC') : 'Error updating avatar');
      }
        ...
        >
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-[8px] text-gray-600 font-mono">ID: {tgUser?.id || userId || '—'}</div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar pb-28 px-3 flex flex-col gap-3 relative z-10">
        ...
        {activeTab === 'general' && (
          <>
            ...
            <div className="glass-card p-3">
              <div className="text-[8px] text-gray-600 uppercase tracking-widest font-bold mb-2">Franchise</div>
              <div className="flex items-center gap-3">
            ...
            <div className="glass-card p-3">
              ...
              {objectives.length > 0 ? (
                ...
                <div className="text-center py-4 text-gray-700 text-[10px] uppercase tracking-widest">Loading objectives…</div>
              )}
            </div>
            ...
            {isAdmin && (
              <Link href="/admin/logs" className="flex items-center justify-center gap-2 w-full py-2.5 rounded-xl
                ...
                <Shield size={11} /> Developer Console
              </Link>
            )}
            ...
            <div className="mt-1">
              <button className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl
                ...
                <UserMinus size={11} /> Resign as Manager
              </button>
            </div>
          ...
          <div className="flex flex-col gap-3">
            <Link href="/achievements"
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-sm font-black text-white uppercase tracking-wider">Hall of Fame</div>
                  <div className="text-[9px] text-gray-500 uppercase tracking-widest">Your achievements</div>
                </div>
              </div>
            ...
            <div className="glass-card p-4 text-center">
              ...
              <div className="text-[10px] text-gray-600 uppercase tracking-widest font-bold">Trophies Coming Soon</div>
              <div className="text-[9px] text-gray-700 mt-1">Win competitions to earn cups & medals</div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'results' && (
          <div className="glass-card p-4 text-center">
            ...
            <div className="text-[10px] text-gray-600 uppercase tracking-widest font-bold">Match History</div>
            <div className="text-[9px] text-gray-700 mt-1">Season results will appear here</div>
        ...
        <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/80 backdrop-blur-md" onClick={() => setShowTerms(false)}>
          <div className="w-full max-w-[480px] glass-card rounded-b-none p-6 relative" onClick={e => e.stopPropagation()}>
            ...
            <div className="text-[10px] text-gray-400 space-y-3 mb-6 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
              <p>In-game currency (FanCoins, SP) has no real-world financial value outside our TON marketplace.</p>
              <p>Developers reserve the right to ban accounts for fraud, step manipulation or forbidden content.</p>
            </div>
            <button onClick={() => setShowTerms(false)} className="w-full py-2.5 bg-violet-500 text-white font-bold uppercase tracking-widest text-xs rounded-xl">Close</button>
          </div>
        </div>
      )}
      ...
      {showDisclaimer && (
        <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/80 backdrop-blur-md" onClick={() => setShowDisclaimer(false)}>
          <div className="w-full max-w-[480px] glass-card rounded-b-none p-6 relative" onClick={e => e.stopPropagation()}>
            ...
            <div className="text-[10px] text-gray-400 space-y-3 mb-6 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
              <p><strong className="text-violet-300">Medical:</strong> This app is for entertainment only and is not a medical device.</p>
              <p><strong className="text-cyan-300">Crypto:</strong> We are not an exchange or broker. TON purchases are in-game transactions.</p>
            </div>
            <button onClick={() => setShowDisclaimer(false)} className="w-full py-2.5 bg-cyan-500 text-black font-bold uppercase tracking-widest text-xs rounded-xl">Close</button>
          </div>
        </div>
      )}
      ...
      {showAvatarModal && (
        <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/80 backdrop-blur-md" onClick={() => setShowAvatarModal(false)}>
          <div className="w-full max-w-[480px] glass-card-violet rounded-b-none p-6 flex flex-col items-center gap-4 relative" onClick={e => e.stopPropagation()}>
            <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-violet-400/60 to-transparent" />
            <h3 className="text-white font-bold text-sm uppercase tracking-widest font-orbitron">Avatar Generator</h3>
            ...
            <div className="w-full">
              <label className="text-[8px] text-gray-500 uppercase tracking-widest mb-1 block">Seed</label>
            ...
            <button onClick={() => setShowAvatarModal(false)} className="text-gray-600 text-[8px] uppercase tracking-widest hover:text-white transition-colors">Cancel</button>
          </div>
        </div>
      )}
    </div>
