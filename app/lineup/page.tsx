...
export default function LineupPage() {
  ...
  return (
    <div className="flex flex-col flex-1 p-4 gap-6 pb-12 bg-space-dark min-h-screen">
      ...
      {/* TACTICAL PITCH UI */}
      <div className="relative w-full aspect-[3/4] bg-green-950/30 border-2 border-neon-green/40 rounded-lg overflow-hidden shadow-[inset_0_0_40px_rgba(57,255,20,0.05)] flex flex-col items-center justify-around py-4">
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-16 h-16 border-2 border-neon-green/20 rounded-full"></div>

        {/* Player Mapping (MVP Grid Layout) */}
        <div className="relative z-10 w-full px-2 grid grid-cols-3 gap-3">
          {activePlayers.map(player => (
            <div key={player.id} className="bg-black/60 backdrop-blur-md border border-neon-cyan/40 p-2 rounded flex flex-col items-center shadow-[0_0_10px_rgba(0,240,255,0.1)] hover:border-neon-cyan transition-colors">
              <span className="text-[10px] text-gray-400 uppercase">Age {player.age}</span>
              <span className="text-xs font-bold text-white truncate w-full text-center mt-1">{player.name}</span>
              <span className="text-lg font-black text-neon-cyan drop-shadow-[0_0_5px_rgba(0,240,255,0.8)]">{player.ovr}</span>
            </div>
          ))}
          {activePlayers.length === 0 && (
            <div className="col-span-3 text-center text-sm text-neon-cyan/60 mt-10 font-mono">No active players on roster</div>
          )}
        </div>
      </div>
